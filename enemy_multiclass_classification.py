...
sf_train = pd.read_csv('datasets/enemy_stats_training.csv')
corr_matrix = sf_train.corr()
sf_train.drop(sf_train.columns[[2, 5, 6, 8, 9]], axis=1, inplace=True)

sf_val = pd.read_csv('datasets/enemy_stats_validation.csv')
...
checkpoint = ModelCheckpoint(
    filepath='./enemy_char_classification.h5',
    monitor='val_loss',
    verbose=0,
    save_best_only=True,
    mode='min'
...
with open('enemy_char_classification.json', 'w') as json_file:
...
df.index = val_data[:,0]
df.to_csv('enemy_char_16_data_result.csv', index=True)
print(df)