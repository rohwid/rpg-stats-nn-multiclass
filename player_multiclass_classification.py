...
sf_train = pd.read_csv('datasets/player_12_training.csv')
corr_matrix = sf_train.corr()
sf_train.drop(sf_train.columns[[8, 11, 14, 17, 20]], axis=1, inplace=True)

sf_val = pd.read_csv('datasets/player_12_validation.csv')
sf_val.drop(sf_val.columns[[8, 11, 14, 17, 20]], axis=1, inplace=True)
...
checkpoint = ModelCheckpoint(
    filepath='./player_char_classification.h5',
    monitor='val_loss',
    verbose=0,
    save_best_only=True,
    mode='min'
...
with open('player_char_classification.json', 'w') as json_file:
...
df.columns = ['Knight', 'Priest', 'Assassin', 'Magician']
df.index = val_data[:,0]
print(df)